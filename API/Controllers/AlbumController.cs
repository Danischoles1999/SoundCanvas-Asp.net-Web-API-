...
namespace API.Controllers
{
    ...
    public class AlbumController : ControllerBase
    {
        ...
        {
            var fetchedAlbum = await _db.Albums.Include(x => x.Artists)
            ...
            fetchedAlbum.PhotoUrl = model.PhotUrl;

            await AssignArtistToAlbumAsync(fetchedAlbum.Id, model.ArtistIds);
            await _db.SaveChangesAsync();
        ...
        public async Task<IActionResult> Create(AlbumAddEditDto model)
        {
            ...
            {
                Name = model.Name,
                PhotoUrl = model.PhotUrl
            };
            _db.Albums.Add(albumToAdd);
            await _db.SaveChangesAsync();

            await AssignArtistToAlbumAsync(albumToAdd.Id, model.ArtistIds);
            await _db.SaveChangesAsync();
        ...
        private async Task AssignArtistToAlbumAsync(int albumId, List<int> artistIds)
        {
